...
* 트랜잭션의 일부를 성공적으로 끝내지 못하면 데이터베이스가 `비일관성(Inconsistency)`인 상태를 가질 수 있기 때문에 <br/> (즉 트랜잭션이 수행한 일부 변경이 데이터베이스에 반영될 가능성이 있기 때문에) 일부분만 완료된 트랜잭션은 <br/> `롤백(Rollback)`되어야 한다.
