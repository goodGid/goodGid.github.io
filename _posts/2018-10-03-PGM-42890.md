...
bool possi(vector<int> &vec,int now){
    /*
     ...
     & 연산을 통해 now값이 후보키가 될 수 있는지 판별한다.
     
     만약
     001이라는 값이 vector에 있다면
     그 뜻은 1번째 컬럼이 후보키라는 뜻이다.
     
     여기서 now가 101이라는 값이라면
...
int solution(vector<vector<string>> relation) {
    ...
    for(int i=1; i<(1<<m); i++){
        set<string> s;
        for(int j=0; j<n; j++){ // j 변수는 각 Line을 가리킨다.
            string now="";
            /*
             k 변수는 각 컬럼을 가리킨다.
             그렇다면 어떻게 활용하느냐?
             ...
             now 변수에 해당 컬럼값을 (+) 해주게 되면
             그 값으로 후보키 자격을 갖출 수 있는지 체크할 수 있게 된다.
             더 자세히 말하면
             일단 1,3번째 값을 합친다음에
             set 컨테이너에 넣는다.
             만약 중복되는값이 있다면 
             set 컨테이너의 size는 n보다 작을 것이다.
             */
            for(int k=0; k<m; k++){
                if( i & (1<<k) )
                    now += relation[j][k];
